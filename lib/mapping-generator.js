...
// @return the mapping
//
function getMapping(cleanTree, inPrefix, explicitEsStorage) {
  var mapping = {},
    value, field, prop,
    implicitFields = [],
    hasEsIndex = false,
  ...
  for (field in cleanTree) {
    ...
    // If using explicit storage, and it has not been specified as an index, set index to no
    if (explicitEsStorage && value.es_stored_field && !value.es_indexed) {
    ...
    if (typeof value === 'object' && !value.type) {
      ...
      mapping[field].properties = getMapping(value, prefix + field, explicitEsStorage);
    }
    ...
    for (prop in value) {
      ...
      if (value.hasOwnProperty(prop) && prop.indexOf('es_') === 0 && prop !== 'es_indexed' && prop !== 'es_stored_field') {
  ...
  if (hasEsIndex && !explicitEsStorage) {
...
// @return the tree
//
function getCleanTree(tree, paths, inPrefix, explicitEsStorage) {
  ...
  for (field in tree) {
    ...
    // If explicit ES field storage has been specified, only store those fields that are marked as an es_stored_field
    if (explicitEsStorage === true && !value.es_stored_field) {
      continue;
    }
    ...
    if (type) {
      ...
      if (typeof value === 'object') {
        cleanTree[field] = getCleanTree(value, paths, prefix + field, explicitEsStorage);
      }
    }
  }
...
function nestedSchema(paths, field, cleanTree, value, prefix) {
  ...
  if (paths[field] && paths[field].schema && paths[field].schema.tree && paths[field].schema.paths) {
    cleanTree[field] = getCleanTree(paths[field].schema.tree, paths[field].schema.paths, '', paths[field].schema.get('es_explicit_storage'));
...
Generator.prototype.generateMapping = function generateMapping(schema, cb) {
  var cleanTree = getCleanTree(schema.tree, schema.paths, '', schema.get('es_explicit_storage')), mapping;
  delete cleanTree[schema.get('versionKey')];
  mapping = getMapping(cleanTree, '', schema.get('es_explicit_storage'));
